...
from agents import Agent, Runner, function_tool, MaxTurnsExceeded, ModelSettings, set_default_openai_client
...
def check_response_format(response: str, num_choices: int) -> bool:
    ...
    """
    valid_answer = False
    if int(response.answer) <= num_choices:
        valid_answer = True
    valid_confidence = False
    if int(response.confidence) >= 1 and int(response.confidence) <= 5:
...
def define_orchestrator(model_name):
    #custom_client = AsyncOpenAI(base_url=os.environ['LITE_LLM_BASE'], api_key=os.environ['LITE_LLM_KEY'])
...
def load_benchmark(name):
    ...
    df = pd.DataFrame(json.load(open(f"/mnt/bulk-sirius/sanddhya/agent_benchmarking/Agent_Edge_Benchmarking/benchmarks/{benchmark_file_map[name]}", 'r'))).set_index('id')
    return df
            ...
            try: 
                ...
                if answer is not None and confidence is not None:
                    ...
                    results.append({
                            ...
                            'reasoning_tokens' : result.context_wrapper.usage.output_tokens_details.reasoning_tokens
                            })
...
def main(args):
    ...
    agent = define_orchestrator(model_name)
